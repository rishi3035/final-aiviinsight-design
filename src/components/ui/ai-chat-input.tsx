...
function ModelIcon({ model, className }: { model: string; className?: string }) {
  const icons: Record<string, string> = {
    ...
    "GPT-4o Search": "https://res.cloudinary.com/drhx7imeb/image/upload/v1781695269/openai-icon_zozuib.svg",
    "Claude 3.7": "https://res.cloudinary.com/drhx7imeb/image/upload/v1781695268/Claude_AI_symbol_yqfzlc.svg",
    "Opus 4.8": "https://res.cloudinary.com/drhx7imeb/image/upload/v1781695268/Claude_AI_symbol_yqfzlc.svg",
    "Perplexity Pro": "https://res.cloudinary.com/drhx7imeb/image/upload/v1781695268/google-gemini-icon_l6kk5q.svg",
    "GLM 5.2": "https://res.cloudinary.com/drhx7imeb/image/upload/v1781695269/z-ai-icon_xi4xvo.svg"
  };

  const filters: Record<string, string> = {
    "GPT 5.5": "dark:invert", 
    "GPT-4o Search": "dark:invert", 
  };
...
function AttachmentThumb({
  ...
  return (
    <button
      ...
      className={cn(
        "group relative size-12 shrink-0 overflow-hidden rounded-xl border border-neutral-700 bg-neutral-800 outline-none",
    ...
    >
      ...
      <span className={cn("absolute inset-0 flex items-start justify-end bg-black/0 transition-colors duration-200", isHovered && "bg-black/25")}>
        <span
          ...
          className={cn(
            "m-1 flex size-4 items-center justify-center rounded-full bg-neutral-900/90 text-neutral-200 shadow-sm transition-all duration-200 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] hover:bg-neutral-900 hover:text-white hover:scale-110",
...
function AttachmentGalleryModal({
  ...
  return (
    <div className="fixed inset-0 z-[100]" onClick={handleClose} role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-black/80 backdrop-blur-md transition-opacity duration-400" style={{ opacity: isOpen ? 1 : 0 }} />
      <div
        style={{
          position: "fixed",
          ...
          boxShadow: isOpen ? "0 24px 60px -12px rgb(0 0 0 / 0.5)" : "0 0px 0px 0px rgb(0 0 0 / 0)",
        }}
      ...
      <button
        ...
        className={cn(
          "fixed right-4 top-4 flex size-9 items-center justify-center rounded-full bg-neutral-800/90 text-white shadow-md backdrop-blur-sm",
          "transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] hover:bg-neutral-700",
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    {
      onSubmit,
      placeholder = "Audit citation authority across AI models...",
      className,
    ...
    const startRecording = useCallback(async () => {
      ...
      function simulateText() {
        const fakeText = "Analyze our enterprise GEO citation authority on ChatGPT Search";
        ...
        >
          ...
          >
            <div
              ...
              className="border border-neutral-800 border-b-0 bg-[#0F141E] rounded-t-2xl px-2 pt-2 pb-1 flex items-start gap-2 overflow-x-auto prompt-scrollbar"
            >
          ...
          <div
            ...
            style={{
              borderRadius: 24,
              minHeight: expanded ? containerHeight : 52,
              transition: isSmoothResize ? SMOOTH_HEIGHT_TRANSITION : SPRING_TRANSITION,
            }}
            className={cn(
              "relative w-full border border-neutral-800 bg-[#0F141E] shadow-xl focus-within:border-emerald-500/40 focus-within:ring-1 focus-within:ring-emerald-500/20 hover:border-neutral-700 z-10 text-white",
          ...
          >
            ...
            <textarea
              ...
              className={cn(
                "prompt-scrollbar absolute top-0 inset-x-0 z-[1] w-full resize-none bg-transparent pl-4 pr-12 py-3.5 text-sm leading-[22px] text-white outline-none placeholder:font-medium placeholder:text-neutral-500 cursor-text font-sans",
            ...
            <div
              ref={topFadeRef}
              className="absolute left-4 right-12 top-0 z-[2] h-8 bg-gradient-to-b from-[#0F141E] via-[#0F141E]/90 to-transparent pointer-events-none"
            />
            <div
              ref={bottomFadeRef}
              className="absolute left-4 right-12 z-[2] h-8 bg-gradient-to-t from-[#0F141E] via-[#0F141E]/90 to-transparent pointer-events-none"
              style={{ 
                opacity: 0, 
            ...
            <div
              className={cn(
                "absolute bottom-2.5 left-3 right-12 z-[10] flex items-center gap-1.5 transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
            ...
            >
              ...
              <div className="relative">
                <button
                  ...
                  className={cn(
                    "group flex items-center gap-1.5 rounded-full px-2.5 py-1 text-neutral-300 transition-all duration-200 outline-none hover:bg-neutral-800 hover:text-white cursor-pointer bg-neutral-900 border border-neutral-800 text-xs font-semibold",
                    isModelSelectOpen ? "bg-neutral-800 text-white ring-1 ring-neutral-700" : ""
                  )}
                  aria-label={`Select model. Current: ${selectedModel}`}
                >
                  <ModelIcon model={selectedModel} className="size-3.5 opacity-90 group-hover:opacity-100 transition-opacity" />
                ...
                <div
                  ...
                  className={cn(
                    "absolute bottom-full left-0 mb-2.5 z-50 w-48 rounded-2xl border border-neutral-800 bg-[#0E121A]/95 p-1.5 shadow-2xl backdrop-blur-md flex flex-col gap-0.5 transition-all duration-400 cursor-default",
                    isModelSelectOpen
                ...
                >
                  <div className="relative flex flex-col gap-0.5">
                    <div style={hoverStyle} className="absolute left-0 right-0 top-0 h-8 -z-10 rounded-xl bg-neutral-800 pointer-events-none" />
                    {models.map((model, idx) => (
                      <button
                        ...
                        onClick={(e) => { 
                          ...
                          else setLocalModel(model);
                          setIsModelSelectOpen(false); 
                        }}
                        className="group relative flex h-8 w-full items-center justify-between rounded-xl px-2.5 py-1.5 text-left text-xs font-medium text-neutral-300 hover:text-white outline-none active:scale-[0.98] cursor-pointer"
                      >
              ...
              {/* Prompt Library Symbol Trigger (Component 2 Integration) */}
              {onOpenPromptLibrary && (
                <button
                  type="button"
                  onMouseDown={(e) => e.preventDefault()}
                  ...
                  className="group flex items-center gap-1.5 rounded-full px-2.5 py-1 bg-neutral-900 border border-neutral-800 hover:border-[#FF7A1A]/50 text-neutral-300 hover:text-white transition-all duration-200 text-xs font-semibold cursor-pointer shadow-2xs hover:bg-neutral-800"
                  aria-label="Open prompt library template modal"
              ...
              <button
                ...
                className="group flex items-center gap-1 rounded-full px-2 py-1 text-neutral-400 hover:bg-neutral-800 hover:text-neutral-200 outline-none cursor-pointer text-xs"
              >
                <DynamicBarsIcon level={efforts[effortIndex]} />
                <span className="text-[11px] font-semibold select-none transition-colors"><MorphingText text={efforts[effortIndex]} /></span>
              </button>

              {/* Attachment Plus */}
              <button
                type="button" onMouseDown={(e) => e.preventDefault()} onClick={openFileChooser} disabled={attachments.length >= maxAttachments}
                className="ml-auto flex size-7 items-center justify-center rounded-full text-neutral-400 hover:bg-neutral-800 hover:text-white outline-none cursor-pointer disabled:opacity-40 disabled:pointer-events-none"
            ...
            <div
              className={cn(
                "absolute right-12 bottom-2.5 z-[10] flex h-8 items-center justify-end gap-[3px] transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
            ...
            >
              {audioData.map((val, i) => (
                <div
                  key={i}
                  className="w-1 rounded-full bg-emerald-400 transition-[height] duration-75 ease-out"
                  style={{ height: `${Math.max(4, val * 24)}px` }}
                />
              ))}
            </div>

            {/* Action Submit / Mic / Stop Button */}
            <button
              type="button"
              onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); }} 
              ...
              className="absolute right-2.5 bottom-2.5 z-[10] flex h-8 w-8 items-center justify-center bg-gradient-to-r from-[#C8102E] to-[#E02444] hover:from-[#B00D27] hover:to-[#C8102E] text-white font-bold transition-all duration-300 hover:scale-105 active:scale-95 outline-none cursor-pointer shadow-md shadow-red-950/20"
            >
