...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(
    <div className={cn("pointer-events-none rounded-xl bg-neutral-900/95 backdrop-blur-md px-3 py-2 text-xs text-white shadow-2xl border border-neutral-700 animate-in fade-in-0 zoom-in-95 duration-200 ease-out z-50", className)}>
      {children}
    </div>,
    container,
  );
